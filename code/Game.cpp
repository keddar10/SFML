...
Game::~Game()
{
	...
	for (int i = NUMBER_OF_HP_POTIONS - 1; 0<=i; --i){
		delete potionHP[i];
	}
	...
	delete slower;
	delete booster;
	for (int i = NUMBER_OF_COINS - 1; 0<=i; --i){
		delete coin[i];
	}
...
void Game::Loop()
{
	...
	while (window->isOpen())
	{
		...
		for (int i = 0; i<NUMBER_OF_HP_POTIONS;i++)
		{
			potionHP[i]->addItem(player, inventory, potionHP[i]);//TODO addItem jako funkcja wirtualna mo¿e przyjmowaæ argumenty jako Item2, np item2& i ka¿da pochodna klasa mo¿e tak sie zachowywaæ
		}
		for (int i = 0; i<NUMBER_OF_MP_POTIONS;i++)
		{
		...
		while (window->pollEvent(event))
		{
			...
			player->useItem(scoreVis, scoreVis, inventory, event, potionMP[0]);//TODO zamiast vector itemow zrobic 1 item, ktory mozna uzywac gdy ilosc w vectorze != 0, wtedy zniknie problem wywolywania potionHP[i] 
			//}			
			//for (int i = 0; i<NUMBER_OF_MP_POTIONS;i++)
			//{
